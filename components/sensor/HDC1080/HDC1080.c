...
static esp_err_t hdc1080_register_write(uint8_t reg_addr, uint16_t data)
{
    ...
    ret = i2c_master_write_to_device(I2C_MASTER_NUM, HDC1080_SENSOR_ADDR, write_buf, sizeof(write_buf), I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
...
static esp_err_t hdc1080_register_read(uint8_t reg_addr, uint16_t *data)
{
    int ret;
    uint8_t read_buf[2];
    ret = i2c_master_write_read_device(I2C_MASTER_NUM, HDC1080_SENSOR_ADDR, &reg_addr, 1, read_buf, sizeof(read_buf), I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
    if (ret == ESP_OK)
    {
...
static esp_err_t hdc1080_data_read_one(uint16_t *data)
{
    int ret;
    uint8_t read_buf[2];
    ret = i2c_master_read_from_device(I2C_MASTER_NUM, HDC1080_SENSOR_ADDR, read_buf, sizeof(read_buf), I2C_MASTER_TIMEOUT_MS / portTICK_RATE_MS);
    if (ret == ESP_OK)
    {
        *data =(uint16_t)((read_buf[0] << 8) | read_buf[1]);  
    }
